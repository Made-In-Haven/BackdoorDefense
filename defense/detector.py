...
class MultiClientDetector:
    CONFIDENCE_MODE_RAW_RATIO = "raw_ratio"
    CONFIDENCE_MODE_BOUNDED_RELATIVE_GAP = "bounded_relative_gap"
    VALID_CONFIDENCE_MODES = {
        CONFIDENCE_MODE_RAW_RATIO,
        CONFIDENCE_MODE_BOUNDED_RELATIVE_GAP,
    }
    ...
    def __init__(
        ...
        tau_corr=0.0,
        confidence_mode=CONFIDENCE_MODE_RAW_RATIO,
        ...
        self.confidence_mode = self._normalize_confidence_mode(confidence_mode)
        self.epsilon = 1e-8
    ...
    @classmethod
    def _normalize_confidence_mode(cls, confidence_mode):
        normalized_mode = str(confidence_mode or cls.CONFIDENCE_MODE_RAW_RATIO).strip().lower()
        if normalized_mode not in cls.VALID_CONFIDENCE_MODES:
            raise ValueError(
                "Unsupported Stage 3 confidence mode '{}'. Supported modes are: {}".format(
                    confidence_mode,
                    ", ".join(sorted(cls.VALID_CONFIDENCE_MODES)),
                )
            )
    ...
    def load_state_dict(self, state_dict):
        ...
        self.confidence_mode = self._normalize_confidence_mode(
            state_dict.get("confidence_mode", state_dict.get("stage3_confidence_mode", self.confidence_mode))
        )
    ...
    def _compute_client_confidences(self, sorted_client_ids, all_class_distance_dict, num_samples, device):
        ...
        for column_id, client_id in enumerate(sorted_client_ids):
            ...
            distance_gap = (second_nearest_distances - nearest_distances).clamp_min(0.0)
            if self.confidence_mode == self.CONFIDENCE_MODE_BOUNDED_RELATIVE_GAP:
                confidence_matrix[:, column_id] = distance_gap / (second_nearest_distances + self.epsilon)
            else:
                confidence_matrix[:, column_id] = distance_gap / (nearest_distances + self.epsilon)
        return confidence_matrix
