...
def config_path_for(stage3_dir):
    ...
    dataset_dir = CONFIG_DIR_MAP.get(stage3_dir.parent.name)
    if not dataset_dir:
        return None
    return repo_root / "configs/avguard/fourclient_full_avguard_fixed_lambda_anchor" / dataset_dir / "stage3_test.json"


def parse_config(stage3_dir):
    config_path = config_path_for(stage3_dir)
    ...
    try:
        ...
        config = {}
    return {
        "dataset": config.get("dataset", stage3_dir.parent.name),
        "client_num": config.get("client_num", ""),
        "attack_client_num": config.get("attack_client_num", ""),
...
def build_rows(root):
    ...
    if not root.exists():
        return rows
    for stage3_dir in sorted(path for path in root.glob("*/stage3") if path.is_dir()):
        row = parse_config(stage3_dir)
        row.update(parse_metric_files(stage3_dir))
        row.update(parse_log(stage3_dir / "experiment.log"))
        rows.append(row)
    return sorted(rows, key=lambda row: (str(row.get("dataset", "")), str(row.get("stage3_result_directory", ""))))
...
def main():
    parser = argparse.ArgumentParser(description="Summarize four-client full AVGuard fixed-lambda-anchor Stage 3 results.")
