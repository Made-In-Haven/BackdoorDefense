...
def config_path_for(stage3_dir):
    ...
    dataset_dir = DATASET_CONFIG_DIR.get(stage3_dir.parent.parent.name, stage3_dir.parent.parent.name.lower())
    return (
        repo_root
        / "configs"
        / "avguard"
...
def find_stage3_dirs(root):
    if not root.exists():
        return []
    return sorted(path for path in root.glob("*/*/stage3") if path.is_dir())
...
def main():
    parser = argparse.ArgumentParser(description="Summarize AVGuard Stage1 ablation Stage 3 results.")
