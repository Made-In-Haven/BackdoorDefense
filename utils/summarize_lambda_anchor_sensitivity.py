...
def lambda_value_from_dir(lambda_dir):
    name = lambda_dir.name
    return name.replace("lambda_anchor_", "", 1) if name.startswith("lambda_anchor_") else ""


def blank_row(stage3_dir):
    return {
        "dataset": stage3_dir.parent.parent.name,
        "lambda_anchor": lambda_value_from_dir(stage3_dir.parent),
...
def config_path_for(stage3_dir):
    ...
    lambda_name = stage3_dir.parent.name
    return (
        repo_root
        / "configs"
        / "avguard"
        / "lambda_anchor_sensitivity"
        / dataset
        / lambda_name
        / "stage3_test.json"
    )
...
def find_stage3_dirs(root):
    if not root.exists():
        return []
    return sorted(path for path in root.glob("*/lambda_anchor_*/stage3") if path.is_dir())
...
def main():
    parser = argparse.ArgumentParser(description="Summarize AVGuard lambda_anchor sensitivity Stage 3 results.")
