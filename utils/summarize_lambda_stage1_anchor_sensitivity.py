...
def blank_row(stage3_dir):
    ...
    lambda_value = lambda_name.replace("lambda_", "", 1) if lambda_name.startswith("lambda_") else ""
    return {
        "dataset": stage3_dir.parent.parent.name,
        "lambda_stage1_anchor": lambda_value,
...
def config_path_for(stage3_dir):
    ...
    return (
        ...
        / "configs"
        / "avguard"
        / "lambda_stage1_anchor_sensitivity"
        / dataset
        / lambda_name
        / "stage3_test.json"
    )
...
def find_stage3_dirs(root):
    if not root.exists():
        return []
    return sorted(path for path in root.glob("*/lambda_*/stage3") if path.is_dir())
...
def main():
    parser = argparse.ArgumentParser(description="Summarize AVGuard lambda_stage1_anchor sensitivity Stage 3 results.")
